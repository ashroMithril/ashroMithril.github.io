...
interface InfiniteGalleryProps {
  ...
  /** Optional poem lines to display */
  poemLines?: string[]
}
...
function GalleryScene({
  ...
  const [currentPoemLine, setCurrentPoemLine] = useState("")
  ...
  useFrame((state, delta) => {
    ...
    planesData.current.forEach((plane, i) => {
      const distanceToCenter = Math.abs(plane.z - halfRange)
      if (distanceToCenter < closestDistance) {
        closestDistance = distanceToCenter
        closestPlaneIndex = plane.imageIndex
      }
    ...
    if (poemLines.length > 0) {
      const lineIndex = closestPlaneIndex % poemLines.length
      setCurrentPoemLine(poemLines[lineIndex] || "")
    }
  })
      ...
      {currentPoemLine && (
        <Html center>
          <div className="pointer-events-none select-none">
            <p className="text-2xl md:text-4xl font-serif text-gray-800 text-center px-4 py-2 bg-white/80 backdrop-blur-sm rounded-lg shadow-lg max-w-2xl">
              {currentPoemLine}
            </p>
          </div>
        </Html>
...
export default function InfiniteGallery({
  ...
  return (
    <div className={className} style={{ ...style, background: "white" }}>
      <Canvas camera={{ position: [0, 0, 0], fov: 55 }} gl={{ antialias: true, alpha: false }}>
        <color attach="background" args={["#ffffff"]} />
        <GalleryScene images={images} poemLines={poemLines} fadeSettings={fadeSettings} blurSettings={blurSettings} />
      </Canvas>
    </div>
  )
}

// Named export for InfiniteGallery
